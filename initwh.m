...
%hinit=(meanv-bg)*max(rand(K,T),eps); % The multiplication by (meanv-bg) is there for getting it into reasonable range.
hinit=bg*max(rand(K,T),eps); % The multiplication by bg is to initialise at the backgroun level. 
